...
export default function IndustryModule({ onLogout }: IndustryModuleProps) {
  ...
  // In-memory local states
  const [industry, setIndustry] = useState<IndustryItem>(INITIAL_INDUSTRIES[0]);
  const [deliveries, setDeliveries] = useState<PickupRequest[]>(
    INITIAL_PICKUP_REQUESTS.filter(p => p.assignedIndustryId === industry.id || p.status === 'Completed')
  );
